...
| S59 | profile/[username] (Nudge chip) | role | Nudge chip beside the gear for a **friend** only, and only when `get_nudge_state` says `ok` (chip) or `cooldown` ("Nudged · 4h", disabled). Every other state is `hidden` and renders NOTHING — a greyed button would itself leak that they're mid-session / opted out / already done | `getNudgeState()` | app/profile/[username]/nudge-button.tsx |
