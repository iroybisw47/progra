...
| S50 | progress-client (home) | nudge | "Your week is ready" recap banner (above the tabs) → opens `/recap?w=` | `props.recapNudge` (set in `loadProgressData` when the week unlocked Sun 6pm local & is unopened) | components/v2/recap-nudge.tsx · components/v2/progress-client.tsx |
...
flowchart TD
  ...
  onb -->|REDESIGN| v2["OnboardingClientV2: welcome→about→goal→categories→habits→calendar"]
  onb -->|legacy| lg["OnboardingClient: 9-step + practice + tours"]
  v2 -->|complete| root
  lg -->|complete| root
  v2 -->|Connect calendar| cal["/auth/google-calendar?from=onboarding"]
  cal -->|"?step=calendar&status"| onb
